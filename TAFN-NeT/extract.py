...
if dataset == 'sysu':
    data_path = '/root/zzf/KMT/Datasets/SYSU-MM01'
    n_class = 395
    test_mode = [1, 2]
elif dataset =='regdb':
    ...
    data_path = '/root/zzf/KMT/Datasets/LLCM'
    n_class = 713
...
def extract_gall_feat(gall_loader):
    ...
    start = time.time()
    ptr = 0
    gall_feat_pool1 = np.zeros((ngall, pool_dim))
    gall_feat_pool2 = np.zeros((ngall, pool_dim))
    gall_feat_pool3 = np.zeros((ngall, pool_dim))
    gall_feat_fc1 = np.zeros((ngall, pool_dim))
    gall_feat_fc2 = np.zeros((ngall, pool_dim))
    gall_feat_fc3 = np.zeros((ngall, pool_dim))
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(gall_loader):
            ...
            gall_feat_pool3[ptr:ptr+batch_num,: ] = feat_pool[batch_num*2:].detach().cpu().numpy()
            ...
            gall_feat_fc3[ptr:ptr+batch_num,: ]   = feat_fc[batch_num*2:].detach().cpu().numpy()
            gall_feat_pool = np.concatenate([gall_feat_pool1, gall_feat_pool2, gall_feat_pool3], axis=1)
            gall_feat_fc = np.concatenate([gall_feat_fc1, gall_feat_fc2, gall_feat_fc3], axis=1)
            ptr = ptr + batch_num
...
def extract_query_feat(query_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(query_loader):
            ...
            # print(query_feat_pool[ptr:ptr + batch_num, :].shape) 64
            # print(feat_pool.shape) 192
            ...
            query_feat_pool = np.concatenate([query_feat_pool1, query_feat_pool2, query_feat_pool3], axis=1)
            query_feat_fc = np.concatenate([query_feat_fc1, query_feat_fc2, query_feat_fc3], axis=1)
            ptr = ptr + batch_num
    print('Extracting Time:\t {:.3f}'.format(time.time()-start))
